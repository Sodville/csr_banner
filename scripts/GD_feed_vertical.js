    ...
    $.get(yql, function(result){
            ...
            .done(function() {
                $('.card.recipient-card').each(function() {
                    ...
                    // Fjern ikon fra time seen
                    $(this).find('.recipient-card-right > .card-timeago > i').remove();

                    //Flytt time seen
                    $(this).find('.recipient-card-right > .card-timeago')
                    .appendTo($(this).find('.card-headshot'));
                ...
                $('.survey-answer-small').each(function () {
                    ...
                    var newText = '';
                    while( newText.length < 200 && wordArray.length !== 0 ) {
